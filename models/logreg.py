...
class LogReg(object):
	...
	def __init__(self,
		...
		# 2. Save given verbose request across all LogReg instances
		LogReg.s_verbose = p_verbose
	...
	def holdout_score(self):
		...
		for index in range(len(self.m_holdout_data)):
			...
			if LogReg.s_verbose and None != self.m_holdout_titles:
		...
		for index in range(len(self.m_holdout_data)):
			...
			if LogReg.s_verbose and None != self.m_holdout_labels:
		...
		for item_type in self.m_holdout_prediction_scores:
			...
			if LogReg.s_verbose:
				print("{0} Holdout prediction accuracy: {1}%%".format(item_type,
					self.m_holdout_prediction_scores[item_type] * 100))

		return self.m_holdout_prediction_scores

	def split_training_and_validation(self):

		if LogReg.s_verbose:
			print("Splitting up training and validation data sets...")

		# 1. Split the given data set into training and test sets and labels
		self.m_training_data, self.m_training_labels, self.m_validation_data, self.m_validation_labels = \
			train_test_split(p_training_and_validation_set, p_training_and_validation_labels,
	...
	def validation_score(self):
		...
		self.m_validationscore = logistic_regression.score(validation_data, validation_labels)
